...
resource "azurerm_network_interface" "firewall-2-port1" {
  ...
  ip_configuration {
    ...
    private_ip_address            = cidrhost(var.sdwan_subnet_prefixes[0], 6)
    primary                       = true
  }
  tags = var.tags
}
...
resource "azurerm_network_interface" "firewall-2-port2" {
  ...
  ip_configuration {
    ...
    private_ip_address_allocation = "Static"
    private_ip_address            = cidrhost(var.sdwan_subnet_prefixes[1], 6)
  }
  tags = var.tags
}
...
resource "azurerm_network_interface" "firewall-2-port3" {
  ...
  ip_configuration {
    ...
    private_ip_address_allocation = "Static"
    private_ip_address            = cidrhost(var.sdwan_subnet_prefixes[2], 6)
  }
  tags = var.tags
}

resource "azurerm_network_interface" "firewall-2-port4" {
  name                          = "firewall-2-port4"
  location                      = data.azurerm_resource_group.sdwan-resource-group.location
  resource_group_name           = data.azurerm_resource_group.sdwan-resource-group.name
  enable_ip_forwarding          = true
  enable_accelerated_networking = var.accelerate == "true" ? true : false
  ...
  ip_configuration {
    ...
    private_ip_address_allocation = "Static"
    private_ip_address            = cidrhost(var.sdwan_subnet_prefixes[3], 6)
...
resource "azurerm_network_interface_security_group_association" "firewall-2-port4nsg" {
  depends_on                = [azurerm_network_interface.firewall-2-port4]
  network_interface_id      = azurerm_network_interface.firewall-2-port4.id
  network_security_group_id = azurerm_network_security_group.publicnetworknsg.id
}
...
resource "azurerm_virtual_machine" "firewall-2" {
  ...
  resource_group_name          = data.azurerm_resource_group.sdwan-resource-group.name
  network_interface_ids        = [azurerm_network_interface.firewall-2-port1.id, azurerm_network_interface.firewall-2-port2.id, azurerm_network_interface.firewall-2-port3.id, azurerm_network_interface.firewall-2-port4.id]
  ...
  os_profile {
    ...
    custom_data = templatefile("${path.module}/config-firewall-2.conf", {
      ...
      port1_ip               = cidrhost(var.sdwan_subnet_prefixes[0], 6)
      port1_mask             = cidrnetmask(var.sdwan_subnet_prefixes[0])
      port2_ip               = cidrhost(var.sdwan_subnet_prefixes[1], 6)
      port2_mask             = cidrnetmask(var.sdwan_subnet_prefixes[1])
      port3_ip               = cidrhost(var.sdwan_subnet_prefixes[2], 6)
      port3_mask             = cidrnetmask(var.sdwan_subnet_prefixes[2])
      port4_ip               = cidrhost(var.sdwan_subnet_prefixes[3], 6)
      port4_mask             = cidrnetmask(var.sdwan_subnet_prefixes[3])
      active_peerip          = cidrhost(var.sdwan_subnet_prefixes[2], 5)
      ...
      forti_router_id        = cidrhost(var.sdwan_subnet_prefixes[1], 6)
