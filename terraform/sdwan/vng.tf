resource "azurerm_public_ip" "pip-vng-1" {
  count               = var.express_route_location == true ? 1 : 0
  name                = "pip-vng-1-${var.asr_virtual_network_name}"
...
resource "azurerm_public_ip" "pip-vng-2" {
  count               = var.express_route_location == true ? 1 : 0
  name                = "pip-vng-2-${var.asr_virtual_network_name}"
  location            = data.azurerm_resource_group.resource-group.location
  resource_group_name = data.azurerm_resource_group.resource-group.name
  allocation_method   = "Dynamic"
}
...
resource "azurerm_virtual_network_gateway" "vng" {
  ...
  ip_configuration {
    ...
    subnet_id                     = module.asr-vnet.vnet_subnets_name_id["GatewaySubnet"]
  }
